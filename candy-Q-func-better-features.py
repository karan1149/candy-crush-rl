...
6.     + average turn score from NSCORES samples
...
STEP_SIZE = 0.0000001		# eta in computing Q_opt
...
def getFeatureVec(state, action):
	...
	aveScore = float(sum(makeSwitch(state, action)[1] for i in xrange(NSCORES)))/NSCORES	
	phi = [minRow, maxRow, sameCol, nValidMoves, maxDelete, aveScore]
	phi += maxCount(state)
	return np.array(phi) 
