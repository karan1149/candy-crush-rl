...
6.     + total utility from NSARS episodes with limited DEPTH
...
STEP_SIZE = 0.000000001		# eta in computing Q_opt
...
def getFeatureVec(state, action):
	...
	totUtil = sum(generateSARSA(state, action) for i in xrange(NSARS))
	phi = [minRow, maxRow, sameCol, nValidMoves, maxDelete, totUtil]
	phi += maxCount(state)
	return np.array(phi) 
